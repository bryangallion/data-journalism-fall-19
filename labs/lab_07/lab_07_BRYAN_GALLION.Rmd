...
```{r}
# Install mapview, ggthemes, scales
...
```{r}
# U.S. territories like Puerto Rico, Guam and and the Virgin Islands weren't found in the population table. Some entries don't include counties -- only states. There's also one county in Arkansas, Montgomery, that isn't found in the population table. These missing entries could cause problems later because it won't provide a 100 percent complete picture of the population.
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per person',title="West Virginia received most pills per person in 2009", subtitle = "Pills per person rate by state", caption = "Source: U.S. Census ACS & ARCOS database") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Northern Midwest, Rust Belt states received an increasing number of pills per person over time", subtitle = "Pills per person by state, 2006-2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
maryland_county_population <- pills_population %>%
  group_by(countyfips, buyer_state, year, buyer_county) %>%
  filter(buyer_state == "MD") %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)

maryland_county_population <- county_geodata_shifted %>%
 inner_join(maryland_county_population, by=c("GEOID" = "countyfips"))
...
maryland_county_population %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Maryland counties received more pills per person over time", subtitle = "Pills per person in Maryland counties, 2006 to 2012", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
```
