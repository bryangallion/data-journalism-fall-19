...
```{r}
#Install Tidyverse
install.packages('tidyverse', repo = "https://www.Tidyverse.org/")

# Install Janitor package
install.packages('janitor', repo ="https://github.com/sfirke/janitor")
...
``` {r}
# Rename reporter DEA number as distributor DEA number in new column
mingo_working <- mingo %>%
...
# Create new reporter DEA number column and input it at start of dataset
mingo_working <- mingo %>%
  mutate(distributor_dea_no = reporter_dea_no) %>%
...
``` {r}
# Remove original reporter DEA number column
mingo_working <- mingo %>%
  mutate(distributor_dea_no = reporter_dea_no) %>%
  select(distributor_dea_no, everything()) %>%
...
``` {r}
# Rename distributor DEA number column as reporter
mingo_working <- mingo %>%
...
``` {r}
# Combine buyer address elements into one column
mingo_working <- mingo %>%
...
``` {r}
# Combine buyer address elements but keep old columns
mingo_working <- mingo %>%
  select(contains("buyer")) %>%
...
``` {r}
# Add space between each column name when combined
mingo_working <- mingo %>%
  select(contains("buyer")) %>%
...
```{r}
# Combine distributor address elements into one column
mingo_working <- mingo %>%
...
# Categorize dosage units by number of pills
mingo_working <- mingo %>%
...
``` {r}
# Categorize into three tiers by dosage unit
mingo_working <- mingo %>%
...
``` {r}
# Create dosage unit categories with case_when function
mingo_working <- mingo %>%
...
``` {r}
# Categorize distributors by in- and out-of-state
mingo_working <- mingo %>%
...
# Summarize the number of shipments to Mingo County pharmacies
mingo %>%
  group_by(buyer_name) %>%
...
# Summarize the number of shipments to Mingo County pharmacies and sort descending
mingo %>%
  group_by(buyer_name) %>%
  summarise(shipments = n()) %>%
...
# Filter to only see pharmacies that received over 1,000 shipments
mingo %>%
  group_by(buyer_name) %>%
  summarise(shipments = n()) %>%
  arrange(desc(shipments)) %>%
...
# Only view pharmacies that received more than 1,000 hydrocodone shipments 
mingo %>%
...
# Group by transaction path from manufacturer to distributor to pharmacy and sort descending
mingo %>%
...
# The most significant combo is Actavis Pharma to Miami-Luken to Tug Valley Pharmacy
...
``` {r}
# Group by distributor name, DEA number, city and address
mingo %>%
...
# The summary table shows the three companies have different DEA numbers and locations
```
...
**Answer** To change the names of the three Cardinal Health facilities, a new column could be created that assesses whether the company is or isn't under the distributor's umbrella. Code could be written to label facilities that include the words "Cardinal Health" in their names, and all other distributors could be labeled saying that they're not part of the organization. This could be done using a case_when function.
