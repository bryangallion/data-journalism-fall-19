...
```{r}
# Save mingo county pills per year as its own object
...
```
```{r}
# Create bar chart that shows change in pills sent to Mingo County from 2006 to 2012  
  ggplot(mingo_county_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="orange") +
  labs(x="Year", y="Total pills", title="In Mingo County, opioids rise from 2006 to 2008, then fall through \n2010 and level out", subtitle = "Total pills shipped to Mingo County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)
```
**Task**: In the tutorial above, you followed the steps to create the graphic with the title: 
...
```{r}
# Create new object that only returns data about pills sent to West Virginia counties in 2012
wv_pills_2012 <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, year, dosage_unit)
```
```{r}
# Create object that only returns data about the population of West Virginia counties in 2012
...
```
```{r}
# Join West Virginia pills and population 2012 datasets 
wv_2012 <- wv_pills_2012 %>%
  inner_join(wv_population_2012, by=("buyer_county")) %>%
  select(buyer_county, dosage_unit, population)
```
```{r}
# Create scatter plot of pills sent to West Virginia relative to population in 2012
ggplot(wv_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total pills in 2012", title="Kanawha County received almost double the number of opioids \nthan the second-highest county relative to population in 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
  geom_text_repel(aes(population, dosage_unit, label=buyer_county),
                  subset(wv_2012, population > 75000))
```
