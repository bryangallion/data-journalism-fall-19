...
```{r}
buyer_details(county = "philadelphia", state="PA",  key=key)
```

```{r}
total_pharmacies_county("philadelphia", state="PA", key=key)
```
```{r}
summarized_county_annual("philadelphia", state="PA", key=key)
```
...
```
```{r}
# Join state_population_per_year and 
kensington %>%
  group_by(buyer_name, buyer_dea_no) %>%
  summarise(shipments = n(),
            total_pills = sum(total_pills), 
            avg_pills_per_shipment = mean(total_pills),
            min_pills_in_shipment = min(total_pills),
            max_pills_in_shipment = max(total_pills)
            ) %>%
  arrange(buyer_name)
```
## Output
