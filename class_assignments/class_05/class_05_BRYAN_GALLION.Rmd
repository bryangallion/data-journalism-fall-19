...
```{r}
# The number of records in buyer_annual_by_year is higher than the records in buyer_totals because the annual dataset tracks purchases per year and the total dataset summarizes all the purchases within the six-year period. The number of records in buyer_addresses is higher than buyer_totals because buyers may have more than one address to which they have pills shipped.
```
...
```{r}
# Combine buyer_addresses and buyer_totals
buyer_address_totals <- buyer_addresses %>%
  right_join(buyer_totals, by="buyer_dea_no") %>%
  arrange(desc(total_pills))

# VA Consolidated Mail Outpatient Pharmacy in North Charleston, South Carolina, received 512,414,034 pills. This pharmacy provides about 80 percent of outpatient prescriptions to veterans and processes over 470,000 prescriptions daily, according to the U.S. Dept. of Veterans Affairs.
...
```{r}
# Combine buyer_address_by_year and buyer_annual_by_year
buyer_address_annual <- buyer_addresses %>%
...
# Rakesh K. Mathur, MD received the most total pills in Maryalnd in 2010: 33,7450. Dr. Mathur had 124,650 more pills than the next-highest doctor in the state that year. Dr. Mathur's license is suspended following conduct that "constituted a substantial likelihood of a risk of serious harm to the public health, safety and welfare based on the physician’s prescribing of opiod medications while failing to use reasonable safeguards or to exercise due diligence," according to the Maryland Board of Physicians.
```
...
# West Virginia received a rate of 464.89 pills per person between 2006 and 2012. 
```
...
```{r}
# Inner join of buyer_totals and buyer_addresses
inner_totals_addresses <- buyer_totals %>%
  inner_join(buyer_addresses, by="buyer_dea_no")

# 148,831 records result from an inner join
```
...
```{r}
# Left join of buyer_totals and buyer_addresses
left_totals_addresses <- buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no")

# The left join also produced 148,831 records, too. The inner and left join may have produced the same records because all the records are found in both datasets, not just in buyer_totals.
```
...
```{r}
# Left join of buyer_totals and buyer_addresses
left_totals_addresses_filter <- buyer_totals %>%
  left_join(buyer_addresses, by="buyer_dea_no") %>%
  filter(buyer_address1 == "is.na(buyer_address1)")

# Receiving 0 rows with this filter could indicate that all the addresses in the buyer_totals and buyer_addresses matched.
```
...
```{r}
# anti_join of buyer_totals and buyer_addresses
anti_totals_addresses <- buyer_totals %>%
...
```{r}
# anti_join of buyer_addresses and buyer_totals
anti_addresses_totals <- buyer_addresses %>%
  anti_join(buyer_totals, by="buyer_dea_no")

# 129,884 resulted from the anti_join of buyer_addresses and buyer_totals, meaning there are buyers in the addresses table that aren't included in the totals table.
